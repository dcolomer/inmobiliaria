...
import ws.PedidoYaCanceladoException_Exception;
...
*/
public class FormCancelarPiso extends AbstractFormBasePagos {

	private static final long serialVersionUID = 1L;
			
	private JButton btnSalir, btnCancelarPiso;
	...
	public static FormCancelarPiso createForm() {			
		return new FormCancelarPiso();
	}
	
	private FormCancelarPiso() {
		super();				
		initGUI();
	}
	...
	private void initGUI() {
		...
		setFrameIcon(new javax.swing.ImageIcon(getClass()
				.getResource("resources/icons/anul16.png")));		
		...
		}			
		{
			...
			lblPedidos.setBounds(314, 19, 124, 18);
		...
		{
			btnCancelarPiso = new JButton();
			getContentPane().add(btnCancelarPiso);
			btnCancelarPiso.setLayout(null);
			btnCancelarPiso.setBounds(17, 112, 85, 42);
			btnCancelarPiso.setName("btnCancelarPiso");
			btnCancelarPiso.setToolTipText("Anular el pedido en curso");
			btnCancelarPiso.setHorizontalTextPosition(SwingConstants.CENTER);
			btnCancelarPiso.setVerticalTextPosition(SwingConstants.BOTTOM);				
			btnCancelarPiso.addActionListener(new ActionListener() {
		...
		}
		{
			...
			btnSalir.setToolTipText("Cerrar la gestion de cancelacion de pisos");
			btnSalir.setHorizontalTextPosition(SwingConstants.CENTER);
			btnSalir.setVerticalTextPosition(SwingConstants.BOTTOM);
			btnSalir.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent evt) {
					try {
						FormPrincipal.setFrmCancelarPisoActivo(false);
						setClosed(true);
	...
	private void cmbNIFClienteItemStateChanged(ItemEvent evt) {
		...
		if (index>-1 && !nif.equals("?"))	{
			...
			pedidos=serviciosInmobiliaria.getPedidosCancelablesByNIF(nif);
			if (!pedidos.isEmpty()) {				
	...
	/*
	 * Manejador para el boton 'Cancelar piso'
	 */
	private void btnCancelarPisoActionPerformed(ActionEvent evt) {
		...
		if (indexPP>-1 && !pedido_sel.equals("?"))	{
			...
			if (indexNF>-1 && !nif_cli.equals("")) {
				...
				// Procedemos a la cancelacion del pedido
				try {
					long num_factura=serviciosInmobiliaria.cancelarPiso(nif_cli, int_pedido_sel);
					
					if (num_factura>0) {
						String msg="Cancelacion realizada correctamente. Factura: "+num_factura+ "\n"
						+"Importe a devolver al cliente: "+Math.abs(serviciosInmobiliaria.getFactura(num_factura).getImporte())+"€";		
						
						JOptionPane.showMessageDialog(this.getParent(), msg, 
								"Resultado de la cancelacion", JOptionPane.INFORMATION_MESSAGE, null);
						
						limpiarPantalla(indexPP);
						
					} else {
						String msg="Ha ocurrido algun problema al cancelar el pedido. num_factura<=0";
					...
					String msg="Cancelacion NO realizada. Comprobar que el pedido no se encuentre ya pagado.";																			
					gestionarError(msg,e);
				} catch (PedidoYaCanceladoException_Exception e) {
					String msg="Cancelacion NO realizada. Comprobar que el pedido no se encuentre ya cancelado.";																			
					gestionarError(msg,e);
				} catch (CajaYaCerradaException_Exception e) {
					String msg="Cancelacion NO realizada. La caja ya se encuentra cerrada.";																			
					gestionarError(msg,e); 
				} catch (Exception e) {					
					String msg="Cancelacion NO realizada.";										
					gestionarError(msg,e);
				}
			} else {
				if (LOG.isDebugEnabled())
			...
			if (LOG.isDebugEnabled())
				LOG.debug("No hay pedidos pendientes de pago (anulables) para el cliente especificado");
		}	
	}

	// Eliminar el pedido del combo - limpiar pantalla -
	private void limpiarPantalla(int indexPP) {		
		cmbPedidosModel.removeElementAt(indexPP);
		if (cmbPedidosModel.getSize()==0) // si ya no quedan pedidos
